...
* **The Home Screen** is a single "chat list" that contains all entities a user interacts with:
    ...
    * External Payees (e.g., `Grocery Store`)
    * Categories (e.g., `Food`)
...
* **Dual-Flow Transactions:** The app seamlessly handles two types of transactions:
    1.  **"Solo Mode" (External):** Paying a payee like `Grocery Store`. This is a one-sided, instant log.
...
| :--- | :--- | :--- | :--- |
| **Backend** | **Python 3.11+** | `uv`, `ruff`, `mypy` | Modern, high-performance tooling. `uv` for speed, `ruff` for all-in-one lint/format, `mypy` for type safety. |
| **API Framework** | **Django** & **DRF** | - | "Batteries-included" for rapid, secure development. Manages auth, admin, and ORM perfectly. |
...
| **Frontend** | **React.js** | **Vite** | The industry-standard SPA framework. Component model is perfect for the chat UI. `Vite` for a fast dev experience. |
...
* **As a user,** I want to create a new `External Payee` (e.g., "Grocery Store") with just a name.
...
* **As a user,** I want to see a "chat list" of all my `Entities` (Friends, Payees, Categories, Total Balance).
...
* **As a user,** when I tap `[Send]` in the `Grocery Store` chat, I want the `Payee` pre-filled, and the transaction to be **`confirmed` instantly** ("Solo Mode").
...
* **Sprint 2: The Money Containers**
    ...
    * **User Stories:** CRUD for `Entity` (Bank, Cash, Payee, Category), CRUD for `Payment_Mode`, Implement server-driven "Smart Setup" (Wallet/App dependency).
...
* **Sprint 3: The "Solo" Transaction**
    ...
    * **User Stories:** Implement "Solo Mode" (External Payee) transactions (instant confirm), Implement Category chat view, Implement "Total Balance" chat view, Implement `[Send]` button in Category chat (pre-filled), Implement "Opening Balance" transaction flow.
...
* **Entity:** The core concept. Any "thing" in the app that can send, receive, or be part of a transaction (e.g., `Account`, `External Payee`, `Category`).
