...
- **Action:** **[Secured]** Listing all users is forbidden.
- **Authentication:** Bearer Token.
- **Response Body (403 Forbidden):**
  ```json
  {
  	"detail": "Listing all users is not permitted."
  }
  ```

### `GET /api/users/me/`

- **Action:** A custom action to retrieve the profile for the _currently authenticated user_. Used by the `AuthContext` and `Profile.tsx`.
- **Authentication:** Bearer Token.
- **Response Body (200 OK):** The full User object for the logged-in user.

### `PATCH /api/users/me/`

- **Action:** A custom action to update the profile for the _currently authenticated user_. Used by `Profile.tsx`.
- **Authentication:** Bearer Token.
...
- **Validation (400 Bad Request):** The backend provides detailed error messages:
  - `{"receiver": ["User not found."]}` (If the username does not exist)
